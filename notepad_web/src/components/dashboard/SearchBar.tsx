    ...
    <header className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6 lg:mb-8">
      <div className="w-full flex-1">
        <label className="flex flex-col w-full">
          <div className="flex w-full flex-1 items-stretch rounded-lg h-full bg-white border border-gray-200 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 dark:bg-[#242424] dark:border-zinc-700 dark:focus-within:border-blue-500 transition-all hover:scale-[1.02] focus-within:scale-[1.02]">
            <div className="pl-3 sm:pl-4 py-4 flex items-center cursor-pointer text-gray-400">
              <SearchIcon fontSize="small" />
            </div>
            <input
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="form-input flex w-full py-4 h-full min-w-0 resize-none overflow-hidden text-gray-800 focus:outline-0 focus:ring-0 border-none bg-transparent placeholder:text-gray-500 px-3 text-sm sm:text-base font-normal leading-normal dark:text-[#E0E0E0]"
